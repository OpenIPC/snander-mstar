...
static int32_t usb_transfer(const char *func, unsigned int writecnt,
			unsigned int readcnt, const uint8_t *writearr, uint8_t *readarr) {
	if (handle == NULL) {
		return -1;
	}
 ...
 *	D7/22	SO/2	(DIN)
 */
static int ch341a_cs_release(bool enable) {
	uint8_t buf[] = {
...
static int ch341a_spi_send_command(unsigned int writecnt, unsigned int readcnt,
...
static int ch341a_spi_init(const char *options) {
	if (handle != NULL) {
	...
	if ((config_stream(CH341A_STM_I2C_750K) < 0) || (ch341a_cs_release(true) < 0)) {
		goto dealloc_transfers;
	}
